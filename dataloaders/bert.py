...
class BertDataloader(AbstractDataloader):
    def __init__(self, args, dataset, distributed, rank, world_size):
        super().__init__(args, dataset, distributed, rank, world_size)
        args.num_items = len(self.smap)
        self.max_len = args.bert_max_len
        self.mask_prob = args.bert_mask_prob
        self.CLOZE_MASK_TOKEN = self.item_count + 1

        # Initialize negative samplers on only rank 0 if distributed
        self.train_negative_samples = None
        self.test_negative_samples = None
        train_negative_sampler = None
        if not dist.is_initialized() and rank == 0:
...
class BertEvalDataset(data_utils.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.negative_samples is None:
            ...
            negs = self.negative_samples[user]  # only train on all ranks
